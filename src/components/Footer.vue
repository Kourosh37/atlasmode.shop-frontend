<script setup>
// ==============================
// Footer: Responsive, With Store Menu
// ==============================
...
// Fetch menu links on mount (once)
onMounted(() => {
  store.fetchMenus();
});
</script>
...
<template>
  ...
  <footer class="bg-gray-200 py-14 mt-16 border-t border-gray-200 px-[10%]" dir="rtl">
      ...
      <div class="w-full md:w-1/3 flex flex-col items-center gap-8 md:gap-6 mb-6 md:mb-0">
        <div class="bg-white/90 rounded-2xl shadow p-6 w-full max-w-sm mx-auto flex flex-col gap-3">
          ...
          <div class="text-sm text-gray-500 text-center mb-2">
            با عضویت از
            <span class="text-indigo-600 font-semibold">تخفیف‌ها</span>
            و
            <span class="text-pink-500 font-semibold">مطالب ویژه</span>
            با خبر شو!
          </div>
          <form class="flex mt-2" @submit.prevent>
            <input
              class="flex-1 rounded-s-xl border border-gray-200 focus:ring-2 focus:ring-indigo-300 outline-none px-4 py-2 text-sm"
              type="email"
              placeholder="ایمیل شما..."
              required
            />
            <button class="rounded-e-xl bg-indigo-600 hover:bg-indigo-700 transition text-white px-5 py-2 text-sm font-bold shadow ml-1">
              عضویت
            </button>
          </form>
        </div>
    ...
    <div class="container mx-auto max-w-7xl text-center text-xs text-gray-500 pt-8 mt-10 border-t border-gray-200">
      ...
      <span class="font-bold text-gray-600">اطلس مد</span>
      می‌باشد. | طراحی با ❤️ توسط
      <span class="text-indigo-500 font-bold">شتاب</span>
    </div>
  </footer>
</template>
