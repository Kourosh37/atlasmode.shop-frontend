...
import { useHomeCategoriesStore } from "../../stores/homeStores/homeCategories";
import { ChevronRight, ChevronLeft, Heart, Star } from 'lucide-vue-next';
...
function onSwiper(swiper, categoryId) {
  swiperInstances.value = { ...swiperInstances.value, [categoryId]: swiper };
}
function slideNext(categoryId) {
  const swiper = swiperInstances.value[categoryId];
  if (swiper) swiper.slideNext();
}
function slidePrev(categoryId) {
  const swiper = swiperInstances.value[categoryId];
...
<template>
  <div class="w-full flex flex-col gap-16" dir="rtl">
    <template v-if="store.loading">
      <div v-for="i in 2" :key="i" class="w-full min-h-[560px] bg-gray-50 py-8 flex flex-col items-center gap-8">
        <div class="container flex justify-between items-center mb-2 px-2 md:px-8">
          <h2 class="text-2xl md:text-3xl font-bold text-gray-700">در حال بارگذاری...</h2>
        </div>
        <div class="container flex gap-6 justify-center overflow-x-auto px-2 md:px-8 py-4">
          <div v-for="j in 3" :key="j" class="bg-white rounded-2xl shadow flex flex-col items-center min-w-[220px] h-[430px]">
            ...
            <div class="flex gap-2 px-6 mt-2 w-full justify-center">
              <div v-for="k in 7" :key="k" class="w-7 h-7 bg-gray-200 rounded-full animate-shine"></div>
            </div>
          </div>
        </div>
      </div>
    ...
    <template v-else>
      <div v-for="cat in store.categories" :key="cat.id" class="w-full min-h-[560px] bg-gray-50 py-8 flex flex-col items-center gap-8">
        <div class="container flex justify-between items-center mb-2 px-2 md:px-8">
          <h2 class="text-2xl md:text-3xl font-bold text-gray-700">{{ cat.title }}</h2>
          <a :href="'/category/' + cat.id" class="font-semibold border-b-2 border-gray-800 text-gray-800 hover:text-blue-500 transition">مشاهده بیشتر</a>
        </div>
        <div v-if="cat.products && cat.products.length" class="w-full flex items-center justify-center max-w-7xl mx-auto relative">
          <button @click="slidePrev(cat.id)"
          ...
          <Swiper
            ...
            @swiper="swiper => onSwiper(swiper, cat.id)"
          >
            <SwiperSlide v-for="product in cat.products" :key="product.id" class="flex flex-col items-center">
              <div
                ...
                @mouseenter="hovered[product.id] = true"
                @mouseleave="hovered[product.id] = false"
              >
