<template>
  <div dir="rtl">
    <!-- لودینگ و پیدا نشدن -->
    <div v-if="productStore.loading" class="flex flex-col items-center justify-center w-full h-60 bg-white/70 backdrop-blur rounded-2xl p-8">
      <svg class="animate-spin h-12 w-12 text-gray-500 drop-shadow-lg mb-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
    ...
    <div v-else-if="productStore.notFound" class="flex flex-col items-center justify-center min-h-[60vh]">
    ...
    <!-- صفحه محصول -->
    <div v-else class="bg-white w-full max-w-4xl mx-auto rounded-2xl shadow-xl p-4 md:p-8 grid grid-cols-1 md:grid-cols-3 gap-7 items-start">
      <!-- تصاویر محصول -->
      <ProductImages />
      <!-- اطلاعات محصول و انتخاب‌ها -->
